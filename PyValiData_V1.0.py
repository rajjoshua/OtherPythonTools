...
class ExcelSQLValidatorApp(QWidget):
    ...
    def init_ui(self):
        ...
        # --- Header ---
        header_label = QLabel("PyValiData")
        ...
        # Load Validation Functions button
        self.load_validation_functions_button = QPushButton("Load Validation Functions (.py)")
        self.load_validation_functions_button.clicked.connect(self.load_validation_functions_file)
        tc_file_layout.addWidget(self.load_validation_functions_button)
        self.validation_functions_path_label = QLabel("No validation_functions.py loaded.")
        tc_file_layout.addWidget(self.validation_functions_path_label)
    ...
    def run_validation(self):
        ...
        validation_lib = getattr(self, 'validation_functions_module', None)
        keyword_lib_missing = needs_keywords and (validation_lib is None)
    ...
    def load_validation_functions_file(self):
        file_dialog = QFileDialog()
        file_dialog.setNameFilter("Python Files (*.py)")
        file_path, _ = file_dialog.getOpenFileName(self, "Load Validation Functions Python File")
        if file_path:
            if os.path.basename(file_path) != "validation_functions.py":
                QMessageBox.critical(self, "Invalid File", "You must select a file named exactly 'validation_functions.py'.")
                self.validation_functions_path_label.setText("No validation_functions.py loaded.")
                self.validation_functions_module = None
                return
            self.validation_functions_path = file_path
            self.validation_functions_path_label.setText(f"Loaded: {os.path.basename(file_path)}")
            # Dynamically import the module
            spec = importlib.util.spec_from_file_location("validation_functions", file_path)
            module = importlib.util.module_from_spec(spec)
            try:
                spec.loader.exec_module(module)
                self.validation_functions_module = module
            except Exception as e:
                QMessageBox.critical(self, "Import Error", f"Could not import validation functions: {e}")
                self.validation_functions_module = None
        else:
            self.validation_functions_module = None
...
class TablePreviewDialog(QDialog):
    def __init__(self, db_conn, parent=None):
        ...
        layout.addWidget(QLabel("Select a table to preview:"))
        ...
        btn_layout.addWidget(self.copy_columns_btn)

        self.insert_sql_btn = QPushButton("Insert SELECT SQL")
        self.insert_sql_btn.clicked.connect(self.insert_select_sql)
        btn_layout.addWidget(self.insert_sql_btn)
        ...
        self.table_list.itemSelectionChanged.connect(self.load_table_preview)
        self.load_tables()
    ...
    def load_table_preview(self):
        selected = self.table_list.currentItem()
        if not selected:
            return
    ...
    def copy_column_names(self):
        ...
        columns = [desc[0] for desc in cursor.description]
        QApplication.clipboard().setText(", ".join(columns))

    def insert_select_sql(self):
        selected = self.table_list.currentItem()
        if not selected:
            return
        table_name = selected.text()
        ...
        QMessageBox.information(self, "SQL Inserted", f"SQL ready to insert:\n\n{sql}")

class DBModeDialog(QDialog):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Select Database Mode")
        self.selected_mode = "disk"
        layout = QVBoxLayout()
        label = QLabel("Choose database mode:")
        layout.addWidget(label)
        self.ram_radio = QRadioButton("RAM (In-Memory, faster, temporary)")
        self.disk_radio = QRadioButton("Disk (Persistent, slower, default)")
